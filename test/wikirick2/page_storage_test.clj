...
(deftest page-storage
  ...
  (testing "select-all-pages"
    (testing-storage "selects all pages"
      ...
      (save-page (create-page storage "FirstPage" "first content") nil)
      (is (= (map :title (select-all-pages storage)) ["FirstPage"]))

      (save-page (create-page storage "SencondPage" "sencond content") nil)
  ...
  (testing "select-recent-pages"
    (testing-storage "selects recent pages"
      (save-page (create-page storage "FirstPage" "first content") nil)
      (save-page (create-page storage "SecondPage" "sencond content") nil)
      (save-page (create-page storage "ThirdPage" "third content") nil)
...
(deftest pape
  (testing "save-page"
    ...
    (testing-storage "increments revisions of a saved page"
      (save-page (create-page storage "FooBar" "some content rev 1") nil)
    ...
    (testing-storage "fails to save an invalid title page"
      (let [page (create-page storage "FooBar" "some content")
        ...
        (is (throw+? (save-page invalid-page nil) [:type :invalid-page-title]))))

    (testing-storage "fails to save same content"
      (save-page (create-page storage "Foo" "foo content") nil)
    ...
    (testing-storage "failed to save, doesn't change recent page results"
      (save-page (create-page storage "Foo" "foo content") nil)
      (save-page (create-page storage "Bar" "bar content") nil)
      (is (throw+? (save-page (select-page storage "Foo") nil) [:type :unchanged-source]))
...
")]
  ...
  (testing "latest-revision"
    (testing-storage "returns the latest revision"
      (save-page (create-page storage "FooPage" "foo content") nil)
  ...
  (testing-storage "page-history"
    (let [before (DateTime/now)]
      (save-page (create-page storage "SomePage" "some content") nil)
      (save-page (create-page storage "SomePage" "foo content") nil)
...
")))))
  ...
  (testing "referred-titles"
    ...
    (testing-storage "forgets old referred titles"
      (let [foo-page (create-page storage "FooPage" "[[BarPage]]")
        ...
        (save-page (assoc foo-page :source "some content") nil)
    ...
    (testing-storage "considers the referred page priority"
      (let [densed-page (create-page storage "Densed" "short content [[TargetPage]]")
        ...
        (save-page densed-page nil)
        (save-page linkful-page nil)
        (save-page sparsed-page nil)
  ...
  (testing "orphan-page?"
    (testing-storage "knows if self is an orphan page or not"
      ...
      (save-page (create-page storage "SomePage" "[[FooPage]]") nil)
